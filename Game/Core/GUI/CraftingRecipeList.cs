...
public partial class CraftingRecipeList : ItemList
{
	...
	[Export] public bool UsePlayerInventory;
		...
		for (int i = 0; i < ItemCount; i++)
		{
			if (IsSelected(i))
			{
				if (UsePlayerInventory)
					Recipes[i].useRecipe(ClientStatics.player.inventory);
				else
	...
	public void LockUncraftableRecipes()
	{
		...
		{
			if (UsePlayerInventory)
				SetItemDisabled(i, !Recipes[i].isRecipePossible(ClientStatics.player.inventory));
			else
